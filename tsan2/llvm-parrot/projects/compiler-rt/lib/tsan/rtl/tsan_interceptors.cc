...
TSAN_INTERCEPTOR(int, __libc_start_main, int (*main) (int, char**, char**), int argc, char** ubp_av, void (*init) (void), void (*rtld_fini) (void), void (* stack_end)){
  Printf("[TSAN]Intercepted __libc_start_main\n");
//  ThreadState *thr = cur_thread();
//  thr->in_rtl = 0;
  TSAN_INTERCEPT(dlopen);
  if(parrot_handle==NULL)
	parrot_handle=REAL(dlopen)("/home/xinan/work/xtern/dync_hook/interpose.so",RTLD_LAZY|RTLD_DEEPBIND);
  CHECK_NE(parrot_handle,NULL);
  Printf("Interpose.so loaded\n");
  get_parrotTurnNum=((u32* (*)())dlsym(parrot_handle,"get_threadLocalTurnNum"));
  Printf("[TSAN-PARROT] get_parrotTurnNum loaded at %0x\n",get_parrotTurnNum);
  PARROT_INTERCEPT(__libc_start_main);
//  thr->in_rtl = 1;
  return PARROT_CALL(int,__libc_start_main,main, argc, ubp_av, init,rtld_fini,stack_end);
}
...
//  int res = REAL(pthread_create)(th, attr, __tsan_thread_start_func, &p);
  PARROT_INTERCEPT(pthread_create);
  int backup=thr->in_rtl;
//  thr->in_rtl=0;
  int res = PARROT_CALL(int,pthread_create,th, attr, __tsan_thread_start_func, &p);
  thr->in_rtl=backup;
  if (res == 0) {
...
TSAN_INTERCEPTOR(int, pthread_join, void *th, void **ret) {
  ...
  PARROT_INTERCEPT(pthread_join);
//  int res = BLOCK_REAL(pthread_join)(th, ret);
  int res = PARROT_CALL(int,pthread_join,th, ret);
  if (res == 0) {
...
TSAN_INTERCEPTOR(void, pthread_exit, void *value) {
	PARROT_INTERCEPT(pthread_exit);
	PARROT_CALL(void,pthread_exit, value);
	return;
}

TSAN_INTERCEPTOR(int, pthread_cancel, void *th) {
	PARROT_INTERCEPT(pthread_cancel);
	return PARROT_CALL(int,pthread_cancel, th);
}
...
//  int res = REAL(pthread_detach)(th);
  PARROT_INTERCEPT(pthread_detach);
  int res = PARROT_CALL(int,pthread_detach,th);
  if (res == 0) {
...
//  int res = REAL(pthread_mutex_init)(m, a);
  PARROT_INTERCEPT(pthread_mutex_init);
  int res = PARROT_CALL(int,pthread_mutex_init,m,a);
  if (res == 0) {
...
TSAN_INTERCEPTOR(int, pthread_mutex_trylock, void *m) {
  ...
  PARROT_INTERCEPT(pthread_mutex_trylock);
  int res = PARROT_CALL(int,pthread_mutex_trylock,m);
...
TSAN_INTERCEPTOR(int, pthread_mutex_timedlock, void *m, void *abstime) {
  ...
  PARROT_INTERCEPT(pthread_mutex_timedlock);
  int res = PARROT_CALL(int,pthread_mutex_timedlock,m,abstime);
...
TSAN_INTERCEPTOR(int, pthread_rwlock_rdlock, void *m) {
  ...
  PARROT_INTERCEPT(pthread_rwlock_rdlock);
  int res = PARROT_CALL(int,pthread_rwlock_rdlock,m);
...
TSAN_INTERCEPTOR(int, pthread_rwlock_tryrdlock, void *m) {
  ...
  PARROT_INTERCEPT(pthread_rwlock_tryrdlock);
  int res = PARROT_CALL(int,pthread_rwlock_tryrdlock,m);
...
TSAN_INTERCEPTOR(int, pthread_rwlock_wrlock, void *m) {
  ...
  PARROT_INTERCEPT(pthread_rwlock_wrlock);
  int res = PARROT_CALL(int,pthread_rwlock_wrlock,m);
...
TSAN_INTERCEPTOR(int, pthread_rwlock_trywrlock, void *m) {
  ...
  PARROT_INTERCEPT(pthread_rwlock_trywrlock);
  int res = PARROT_CALL(int,pthread_rwlock_trywrlock,m);
...
TSAN_INTERCEPTOR(int, pthread_rwlock_unlock, void *m) {
  ...
  PARROT_INTERCEPT(pthread_rwlock_unlock);
  int res = PARROT_CALL(int,pthread_rwlock_unlock,m);
...
TSAN_INTERCEPTOR(int, pthread_cond_timedwait, void *c, void *m,
  ...
  PARROT_INTERCEPT(pthread_cond_timedwait);
  int res = PARROT_CALL(int,pthread_cond_timedwait,c,m,abstime);
...
TSAN_INTERCEPTOR(int, pthread_barrier_init, void *b, void *a, unsigned count) {
  ...
  PARROT_INTERCEPT(pthread_barrier_init);
  int res = PARROT_CALL(int,pthread_barrier_init,b,a,count);
...
TSAN_INTERCEPTOR(int, pthread_barrier_destroy, void *b) {
  ...
  MemoryWrite(thr, pc, (uptr)b, kSizeLog1);
  PARROT_INTERCEPT(pthread_barrier_destroy);
  int res = PARROT_CALL(int,pthread_barrier_destroy,b);
...
TSAN_INTERCEPTOR(int, pthread_barrier_wait, void *b) {
  ...
  PARROT_INTERCEPT(pthread_barrier_wait);
  int res = PARROT_CALL(int,pthread_barrier_wait,b);
...
TSAN_INTERCEPTOR(int, sem_init, void *s, int pshared, unsigned value) {
  ...
  PARROT_INTERCEPT(sem_init);
  int res = PARROT_CALL(int,sem_init,s,pshared,value);
...
TSAN_INTERCEPTOR(int, sem_wait, void *s) {
  ...
  PARROT_INTERCEPT(sem_wait);
  int res = PARROT_CALL(int,sem_wait,s);
...
TSAN_INTERCEPTOR(int, sem_trywait, void *s) {
  ...
  PARROT_INTERCEPT(sem_wait);
  int res = PARROT_CALL(int,sem_trywait,s);
...
TSAN_INTERCEPTOR(int, sem_timedwait, void *s, void *abstime) {
  ...
  PARROT_INTERCEPT(sem_timedwait);
  int res = PARROT_CALL(int,sem_timedwait,s,abstime);
...
TSAN_INTERCEPTOR(int, sem_post, void *s) {
  ...
  PARROT_INTERCEPT(sem_post);
  int res = PARROT_CALL(int,sem_post,s);
...
void InitializeInterceptors() {
  ...
  TSAN_INTERCEPT(__libc_start_main);
  //LOAD_PARROT;
