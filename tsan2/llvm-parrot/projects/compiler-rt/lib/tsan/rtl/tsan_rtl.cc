...
u32 CurrentStackId(ThreadState *thr, uptr pc) {
  if (thr->shadow_stack_pos <= (uptr*)0x100 || thr->shadow_stack_pos >=(uptr*)0xffffff00)
	  return 0;
  if (thr->shadow_stack_pos == 0)  // May happen during bootstrap.
    return 0;
  if (pc) {
	  Printf("CurrentStackAddr:%0x\n",thr->shadow_stack_pos);
...
static void MemoryRangeSet(ThreadState *thr, uptr pc, uptr addr, uptr size,
  ...
  if (kGoMode || size < 64*1024) {
    ShadowValue *p = (ShadowValue*)MemToShadow(addr);
    ...
    ShadowValue *begin = (ShadowValue*)MemToShadow(addr);
    ShadowValue *end = begin + size / kShadowCell * kShadowCnt;
    ShadowValue *p = begin;
...
void MemoryResetRange(ThreadState *thr, uptr pc, uptr addr, uptr size) {
  MemoryRangeSet(thr, pc, addr, size, *((u64*)(&SV_Zero)));
}
...
#endif
#if 0
  Printf("FuncEntry->thr:%08x\n",thr);
  Printf("FuncEntry->thr->shadow_stack_pos:%0x\n",thr->shadow_stack_pos);
  Printf("FuncEntry->thr->shadow_stack:%0x\n",thr->shadow_stack);
  Printf("FuncEntry->thr->shadow_stack_end:%0x\n",thr->shadow_stack_end);
  Printf("FuncEntry->pc:%08x\n",pc);
#endif
  thr->shadow_stack_pos[0] = pc;
  thr->shadow_stack_pos++;
}
